...
const ExpenseTracker = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!auth.currentUser || !description || !amount) return;
                ...
                <div className="p-6">
                    ...
                    <div className="flex justify-between mb-6">
                        ...
                        <div className="w-1/2 text-center p-4 bg-red-100 rounded-lg">
                            ...
                            <p className="text-lg font-bold text-red-600">{formatCurrency(Math.abs(expenses))}</p>
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        className={`bg-blue-500 text-white px-4 py-2 rounded w-full mb-4 ${transactions.length === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
                            ...
                            <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
                                ...
                                <div className="space-y-2">
                                    {displayedTransactions.map(transaction => (
                                        <div key={transaction.id} className="grid grid-cols-[0.8fr_0.8fr_1.5fr_0.1fr] gap-1 p-3 border rounded-lg items-center bg-white shadow">
                                            <span className="font-medium truncate">{transaction.description}</span>
                                            <span className="text-sm text-gray-500">{formatDate(new Date(transaction.timestamp))}</span>
                                            <span className={clsx(transaction.amount > 0 ? "text-green-500" : "text-red-500")}>
                                                {transaction.amount > 0 ? `+ ${formatCurrency(transaction.amount)}` : `- ${formatCurrency(Math.abs(transaction.amount))}`}
                                            </span>
                                ...
                                <div className="flex justify-between mt-4">
                                    <button disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)} className="text-blue-500 disabled:opacity-50">Previous</button>
                                    <span>Page {currentPage} of {totalPages}</span>
                                    <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(currentPage + 1)} className="text-blue-500 disabled:opacity-50">Next</button>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSubmit}>
                        <h2 className="text-lg font-semibold text-gray-900 mb-4 border-b-2 border-gray-300">Add new transaction</h2>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Description</label>
                                <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} maxLength={20} required
                                    className="mt-1 block w-full px-4 py-3 rounded-md border border-gray-300 shadow-sm focus-visible:outline-2  focus-visible:outline-blue-500" placeholder="Enter Short description..." />
                            </div>
                            <div>
